...
- **active** — `--ink` (иконка + подпись) + пип (точка) `~5px` под подписью, заливка `--ink`. Активная вкладка определяется текущим роутом (`usePathname`).
- **inactive** — `--ink-30` (иконка + подпись), пип прозрачный.
