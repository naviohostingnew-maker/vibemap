...
# VibeMap · Aurora — расширение v0.2 (вайб-соцсеть + Volly)
...
|---|---|---|
| `--accent-tint` | `rgba(212,83,126,0.12)` | Розовая подложка чипов Volly и тега «по вайбу» (§16). Alpha — композитится поверх mesh/cream. |
| `--surface-volly` | `#ffffffcc` | Поверхность голосового баббла Volly и in-chat карточек (§14–15). Полупрозрачная, **без** backdrop-blur. |
...
> а `Sparkles` — сквозной знак ИИ во всём продукте и потому уходит на центр-вкладку Volly.
> Лента берёт `Newspaper`. На `proto/usp` действует таблица выше.

### Центральная вкладка «Volly» — приподнятый акцент
...
- Подпись «Volly» под кругом, `11px`, активная `--ink`/500, неактивная `--ink-30`.
...
## 13. Контекстная строка «Спроси Volly»

> Полоса вызова Volly на контент-экранах (Лента, Каталог, Знакомства). Не glass-карточка —
> это inline-control. Volly живёт тремя способами (UX-спек): центр-вкладка, **эта строка**,
> голос в карточках.
...
- **Ведущая иконка** (`18px`, `--accent-magenta`):
  - `Sparkles` — когда тап по строке открывает чат Volly (Лента, Знакомства).
...
## 14. Голосовые бабблы (Volly-чат)

> Чат Volly: реплики юзера и Volly — голосовые. Бабблы — **chrome чата, не glass (§4)**:
...
Текстовая реплика Volly — `body` (13px/1.5), `--ink`, `margin-top: 6px` под волной.
...
- Транскрипт **скрыт**; раскрытие — тап по бабблу. Раскрытый текст — `body` (13px/1.5),
  у юзера белый, у Volly `--ink`.
- Ответ Volly проигрывается **автоматически** (тап «отправить» юзера — свежий user-gesture,
...
### 14.5 Шапка чата Volly

Хедер `/volly`: слева аватар-круг `26×26` (`--accent-magenta` + `Sparkles` белый) + «Volly»
...
> Volly отдаёт результаты **карточками прямо в баббл** (UX-спек: не текстом). Тот же
> family-паттерн на двух плотностях: компактная (in-chat) и полная (каталог/лента).

### 15.1 Компактная карточка (внутри баббла Volly)
...
### 16.1 Seed-чип / тег «по вайбу» (подсказки Volly)
...
- Seed-чип — **тап-таргет**: тап вкидывает слово в строку Volly / запрос.
...
- **Строка пред-заполнения Volly**: иконка `Sparkles` (13px, `--accent-magenta`) +
  пояснение `micro`/`--ink-50` («Volly подставил 20:00 — под „вечер“ из запроса»).
...
- Подтверждение: шит закрывается → реплика-подтверждение прилетает в чат Volly +
...
> «почему рекомендовано» в Ленте. Несёт голос Volly (§9). Это **единственное** место, где
...
- Тело — реплика в тоне Volly (§9: italic-friendly, genderless, first-person),
...
| `<VoiceBubble variant="user\|volly">` | Volly-чат | — |
| `<ResultCard density="compact\|full">` | Volly-чат | Каталог, Лента, профиль друга |
